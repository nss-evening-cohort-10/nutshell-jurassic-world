...
const individualRideLoggedIn = (ride) => {
  const domString = `<div class="ride-card card col-md-3 m-3">
    ...
    <button id="edit-${ride.id}" data-toggle="modal" data-target="#rideUpdateModal" class="ride-edit btn btn-dark cudButton">Update Ride</button>
    <button id="delete-${ride.id}" class="ride-delete btn btn-dark cudButton">Shutdown Ride</button>
    </div>
  </div>`;
  return domString;
};
